...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    """
    # Load image and mask
    image, annotations = dataset.data_creator.create_training_batch(batch_size=1)
    image = image[0]
    annotations = annotations[0]
    image = np.asarray(image)
    original_shape = image.shape
    mask, class_ids = dataset.load_mask(annotations=annotations,
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            keras.callbacks.CSVLogger(os.path.join(self.log_dir, "training_log.csv"), separator=",", append=True)
        ]
